  ...
  "edges": [ {{ "from": "node_1", "to": "node_2", "dep_type": "Data" }} ]
    ...
    async fn summarize(&self, input: &AgentInput, _registry: &dyn SystemRegistry) -> AgentOutput {
        ...
        let prompt = format!(
            ...
            If the retrieved data is macroscopic, irrelevant SEO garbage, or completely empty, EXPLICITLY state the specific data deficiency directly (e.g., 'No specific news found for today') INSTEAD of hallucinating misaligned fluff or summarizing a generic yearly calendar. \
