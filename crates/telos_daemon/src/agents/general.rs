...
impl ExpertAgent for GeneralAgent {
    async fn plan(&self, input: &AgentInput, _registry: &dyn SystemRegistry) -> AgentOutput {
        ...
        let tools_str = if tools.is_empty() {
            "No specialized tools found. Use general reasoning.".to_string()
        } else {
            tools.iter()
        ...
        let system_prompt = format!("{}{}{}", env_context, mem_context, format!(r#"You are the GeneralAgent expert for the Telos system. Your job is to solve the user's task by planning a sequence of tool calls.
...
3. ANALYZE THE AVAILABLE TOOLS AND SELECT THE BEST ONES TO ACCOMPLISH THE TASK.
...
{{
  "nodes": [
    {{ "id": "search_1", "agent_type": "tool", "task": "web_search", "schema_payload": "{{\"query\": \"Apple AAPL current stock price\"}}" }},
