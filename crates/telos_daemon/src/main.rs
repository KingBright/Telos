        ...
        while let Some(event) = event_rx.recv().await {
            match event {
                AgentEvent::UserInput { session_id, payload, trace_id } => {
                    ...
                    execution_engine.run_graph(&mut graph, &empty_ctx, registry.as_ref(), broker_bg.as_ref()).await;
                ...
                AgentEvent::UserApproval { task_id, approved, .. } => {
                    ...
                    broker_bg.publish_feedback(AgentFeedback::Output {
                        ...
                        content: if approved { "Task Approved and Continuing...".into() } else { "Task Rejected.".into() },
                        is_final: true,
                    });
                }
                _ => {}
